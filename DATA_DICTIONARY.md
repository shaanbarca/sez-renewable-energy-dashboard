...
| [fct_site_resource](#31-outputsdataprocessedfct_site_resourcecsv) | fact | 81 | PVOUT at centroid + best-within-50km; capacity factors; buildability columns (NaN until `data/buildability/` populated) | `dim_sites` · Global Solar Atlas GeoTIFF · optional: `data/buildability/` | Answers "how much sun does each site get?" — and (when data available) "how much of that land is actually buildable?". | ⚠️ `pvout_buildable_best_50km` NaN until data/buildability/ populated (run `scripts/download_buildability_data.py`) |
...
**Rows:** 7 (2 aluminium + 5 fertilizer)

**Reproducibility rule:** Site selection for sectors with available trackers (cement, steel, nickel IIA) is derived programmatically inside `src/pipeline/build_industrial_sites.py` — those rows are NOT in this CSV. This file is the fallback for sectors without a tracker step (currently aluminium and fertilizer). The fertilizer set is audit-trailed by `data/industrial_sites/fertilizer_universe_v1.csv` (4-source universe-discovery pass: state holdings + APPI + ESDM gas allocations + company websites). Open tracker integration item: TODO M25 (aluminium GEM GAST). Fertilizer closed as of 2026-04.
...
| `best_solar_site_dist_km` | float | computed | Haversine distance (km) from site centroid to chosen patch. Always ≤ 50km. NaN when data absent. |
| `solar_search_method` | str | computed | V3.7: `"substation_anchored"` (a buildable patch within 10km of an existing substation met the 30% demand-coverage floor and won on LCOE proxy) or `"best_pvout_fallback"` (no nearby patch qualified, reverted to argmax PVOUT). Only fallback should produce `invest_substation` labels. |
| `chosen_anchor_substation_name` | str | computed | V3.7: Name of the substation anchoring the chosen patch. Null when `solar_search_method == "best_pvout_fallback"`. |
| `solar_supply_share_pct` | float | computed | V3.7: Chosen patch nameplate MWp ÷ required_mwp, capped at 1.0. |
| `solar_delivered_share_pct` | float | computed | V3.7: Chosen patch annual generation MWh ÷ demand MWh, capped at 1.0. CF-corrected version of supply share — the honest answer to "how much of this site's 2030 demand can actually be met by this patch?" |
| `project_scale_solar_mwp` | float | computed (**live-derived in scorecard**) | V3.7: `min(required_mwp × meaningful_share_pct, max_buildable_mwp)` — the capacity actually assumed to get built to cover site demand. Re-derives live from the `meaningful_share_pct` slider in the Assumptions panel, so moving the slider flips `capacity_assessment`, `substation_upgrade_cost_per_kw`, and the "Upgrade Substation" infrastructure label in real time. The pipeline still writes a default value (at `MEANINGFUL_SHARE_PCT = 0.30`) for backward compatibility; the scorecard enricher overwrites it. Used for substation capacity checks and LCOE grid-connected sizing. |
| `within_boundary_area_ha` | float | computed | Buildable area (ha) inside the site polygon (KEK only — non-KEK sites have no polygon, so this is 0.0) after the full 4-layer buildability filter (slope + Kawasan Hutan + peat + land cover). **0.0 when spatial intersection finds no buildable pixels** (no theoretical fallback). v4.0.5: this is the **baseline** (strict-filter) area; pair with `within_boundary_hard_max_ha` to compute slider-aware deployable area client-side. See METHODOLOGY §5.1.1. |
| `within_boundary_capacity_mwp` | float | computed | `within_boundary_area_ha / 1.5` — max captive solar capacity inside site boundary (MWp) under the strict 4-layer filter. **Baseline value** (slider-independent). 0.0 when area is 0 or no polygon exists. Frontend computes deployable = baseline + (hard_max - baseline) × land-use-override-slider%; see METHODOLOGY §5.1.1. |
| `within_boundary_hard_max_ha` | float | computed | v4.0.5 (methodology #40): polygon area passing **only HARD filters** (slope + Kawasan Hutan + peat — drops land cover). Represents land that's physically and legally buildable for solar regardless of zoning. Invariant: `hard_max_ha >= area_ha` by construction. Used by the land-use override slider to interpolate the deployable display value. |
| `within_boundary_capacity_hard_max_mwp` | float | computed | v4.0.5: `within_boundary_hard_max_ha / 1.5` — capacity ceiling reachable when the site owner overrides all soft-zoning exclusions (slider = 100%). Used client-side + by `dash/logic/grid.py:112` to drive the within_boundary self-sufficiency gate via the slider override math. |
| `pvout_within_boundary` | float | computed | Mean annual PVOUT (kWh/kWp/yr) of buildable pixels inside the site polygon. NaN when no buildable pixels exist or site has no polygon. Used for both baseline and hard_max generation calculations (per methodology #40 §5: sub-5% in-polygon variation at 1km Indonesian resolution makes reusing PVOUT a defensible simplification). |
...
**Rows:** 81 ✅ (25 KEK area-based + 46 standalone sector-intensity + 10 cluster sector-intensity)
...
**Rows:** 1,458 (81 sites × 9 WACC values × 2 scenarios) ✅ — general form: `len(dim_sites) × 9 × 2`
...
**Rows:** 81 ✅ (25 KEK + 46 standalone + 10 cluster)
...
6. CBAM exposure detection dispatched via `SITE_TYPES[site_type].cbam_method`: `3_signal` (KEK) vs `direct` (standalone/cluster/KI). 68/81 sites exposed (12 KEK + 56 industrial direct).
...
| `lcoe_grid_connected_capped_usd_mwh` | float | V3.7: `min(lcoe_grid_connected_usd_mwh, PERPRES_112_CEILING_USD_MWH)` when `solar_regime == "grid_connected_ipp"`, otherwise unchanged. `PERPRES_112_CEILING_USD_MWH = 75.0`. Represents the regulated IPP tariff ceiling; captive and unclear regimes are not ceiling-bound. |
| `solar_regime` | str | V3.7: `co_located_captive` / `grid_connected_ipp` / `unclear`. Heuristic: `within_boundary_coverage_pct ≥ 0.30` OR Perpres-112 exempt industrial sector → `co_located_captive`; `grid_integration_category in {invest_transmission, invest_substation, grid_first}` → `grid_connected_ipp`; else `unclear`. Drives which regulatory regime applies to the LCOE. |
...
| `delivered_cost_usd_mwh` | float/null | V3.11: **Supply Blend** — tenant-view cascaded cost of electricity. Three-layer cascade: `captive × wb_LCOE + remote × gc_LCOE + grid × grid_rate` where solar is physically capped at `daytime_cap = SOLAR_PRODUCTION_HOURS / 24 ≈ 0.417`. See METHODOLOGY §5.4. Null when `grid_cost_usd_mwh` is missing or both `lcoe_within_boundary` and `lcoe_mid` (grid-connected) are missing. Additive metric — does NOT replace `lcoe_mid_usd_mwh`; action flags / economic tier / competitive gap are unchanged unless the CostBasis toggle is set to `delivered`. |
| `captive_fraction` | float/null | V3.11: Within-boundary solar share. `min(within_boundary_coverage_effective_pct, daytime_cap)`. Capped at the daytime ceiling even when WB coverage exceeds it. |
| `delivered_cost_dispatchable_re_fraction` | float/null | F1 (2026-05-07): Dispatchable RE share — geothermal (full-uniform 24h) or hydro (near-uniform). `min(dispatchable_re_coverage_pct, 1 − captive_fraction)` when the upstream `dispatchable_re_coverage_pct` and `dispatchable_re_lcoe_usd_mwh` columns exist (populated by F2 geothermal proximity matching and v4.1b hydro extensions); null otherwise. Sits between within-boundary solar and remote IPP in the cascade — competes with grid backfill for nighttime demand and with both solar layers for daytime. |
| `delivered_cost_dispatchable_re_lcoe_used_usd_mwh` | float/null | F1: Echo of the dispatchable RE LCOE fed into the cascade. Null when the dispatchable RE layer is a no-op. |
| `delivered_cost_remote_fraction` | float/null | V3.11 + F1: Remote captive solar share (grid-connected IPP within ~50 km with gentie). `max(daytime_cap − captive_fraction − f_dispatchable_re × daytime_cap, 0)` when a grid-connected siting exists (`gc_row` present), else 0. F1 changed the formula to subtract dispatchable RE's proportional daytime share to avoid double-counting; bit-identical to v4.0 when dispatchable RE columns are absent (`f_disp_re = 0`). |
| `grid_fraction` | float/null | V3.11: `1 − captive_fraction − delivered_cost_remote_fraction`. Share of demand served by PLN. By construction ≥ `1 − daytime_cap ≈ 0.583` — night demand always goes to grid. |
| `delivered_cost_grid_rate_used_usd_mwh` | float/null | V3.10: Echo of the grid rate fed into the blend — equals the I-4/TT industrial tariff in default mode, or regional BPP in BPP benchmark mode. Exposed for UI narration. |
| `delivered_cost_wb_lcoe_used_usd_mwh` | float/null | V3.10: Echo of the within-boundary LCOE fed into the captive layer. Same value as `lcoe_within_boundary_usd_mwh`, duplicated for pipeline-input transparency. Null if no WB row. |
| `delivered_cost_gc_lcoe_used_usd_mwh` | float/null | V3.11: Echo of the grid-connected LCOE fed into the remote captive layer. Same value as `lcoe_mid_usd_mwh`, duplicated for pipeline-input transparency. Null if no GC row. |
| `delivered_cost_gap_vs_grid_pct` | float/null | V3.10: `(delivered_cost_usd_mwh − grid_rate) / grid_rate × 100`. Negative = delivered cost below grid. Tenant-view counterpart to `solar_competitive_gap_pct`. |
...
| `recommended_grid_link_section` | str/null | F5, 2026-05-09 | RUPTL section reference for the region's representative link (e.g. `V.9.4`). Null when region has no flagged links. |
| `comparator_feasibility` | str enum | F5, 2026-05-09 | Whether the PLN-tariff comparator is realistic for this site: `pln_tariff_feasible` (default) / `pln_tariff_uncertain_grid_first_required` (region-rolled `under_study` / `cross_border` link AND site needs new transmission) / `pln_tariff_infeasible_captive_only` (region has `not_feasible` link AND site needs new transmission). Informational today; action-flag flip deferred to follow-up. |
...
| Column | Type | Source | Formula / Notes |
|--------|------|--------|-----------------|
| `cbam_exposed` | bool | Derived | True if site has CBAM-covered products. Detection method dispatched via `SITE_TYPES[site_type].cbam_method`: **3_signal** (KEK) — (1) nickel process types (RKEF/FeNi), (2) plant-level counts (steel/cement), (3) KEK business sectors; **direct** (standalone/cluster/KI) — uses `cbam_product_type` from `dim_sites.csv`. **68/81 sites exposed** (12 KEK via 3-signal + 56 industrial via direct: 32 cement + 17 iron_steel + 5 fertilizer + 2 aluminium). |
...
| `cbam_savings_2034_usd_per_tonne` | float/null | Derived | CBAM savings per tonne from switching to solar in 2034. Scope 2 savings are multiplied by `CBAM_RE_ADDRESSABLE_FRACTION[product_type]` (cement 0.12, fertilizer/ammonia 0.10, steel_bfbof 0.80, else 1.0) to reflect the electric share of thermal-inclusive intensity values. Scope 1 cost is unaffected. |
...
| `scope1_abatement_pathways` | str/null | F9 (2026-05-07): Comma-separated names of non-RE Scope 1 abatement pathways for the primary CBAM product type. Cement → `"alt_fuels,scm_substitution,electric_kiln"`, ammonia/fertilizer → `"green_h2_smr"`, steel_bfbof → `"hydrogen_dri,scrap_substitution"`, steel_eaf → `"scrap_substitution"`, aluminium → `"inert_anodes"`, nickel RKEF / non-CBAM → null. Sourced from IEA / IRENA / Mission Possible Partnership sectoral roadmaps. Cost is NOT modeled — qualitative availability flag only. |
| `scope1_abatement_indicative_addressable_pct` | float/null | F9: Additional Scope 1 fraction reachable via the listed pathways. Cement 0.30 / ammonia 0.50 / fertilizer 0.50 / steel_bfbof 0.70 / steel_eaf 0.10 / aluminium 0.10 / nickel_rkef 0.0. Surfaces that the `CBAM_RE_ADDRESSABLE_FRACTION` ceiling isn't a static long-term limit. Null for non-CBAM-exposed sites. |
| `scope1_abatement_methodology_note` | str/null | F9: One-line caveat — `"Indicative — full cost modeling deferred to v5.x. Pathway availability is qualitative; the dashboard does not model the levelized cost of these abatement paths today."` Lives next to the ScoreDrawer Industry tab badge. |
...
**Rows:** 81 (one per site — 25 KEK + 46 standalone + 10 cluster)
