...
| `delivered_cost_remote_fraction` | float/null | V3.11: Remote captive solar share (grid-connected IPP within ~50 km with gentie). `daytime_cap − captive_fraction` when a grid-connected siting exists (`gc_row` present), else 0. Fills the daytime headroom up to the physical ceiling. |
