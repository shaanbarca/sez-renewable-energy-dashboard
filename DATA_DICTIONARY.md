...
| `delivered_cost_usd_mwh` | float/null | V3.10: Tenant-view blended cost of electricity when on-site solar partially covers demand. `captive_fraction × lcoe_within_boundary + grid_fraction × grid_cost_usd_mwh`. See METHODOLOGY §5.4. Null when `lcoe_within_boundary` or `grid_cost_usd_mwh` is missing. Additive metric — does NOT replace `lcoe_mid_usd_mwh`; action flags / economic tier / competitive gap are unchanged. |
| `captive_fraction` | float/null | V3.10: `min(within_boundary_coverage_effective_pct, 1.0)`. Haircut-adjusted share of demand served by on-site captive solar, matching the `within_boundary` gate logic in §8.2. |
| `grid_fraction` | float/null | V3.10: `1 − captive_fraction`. Share of demand served by PLN. |
...
| `delivered_cost_wb_lcoe_used_usd_mwh` | float/null | V3.10: Echo of the within-boundary LCOE fed into the blend. Same value as `lcoe_within_boundary_usd_mwh`, duplicated for pipeline-input transparency. |
