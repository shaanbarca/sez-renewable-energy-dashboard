...
export default function ActionFlagLegend() {
  ...
  // Position the portal panel below the trigger. Recompute on open, resize,
  // and scroll so the panel tracks the button.
  useLayoutEffect(() => {
    if (!open) return;
    const update = () => {
  ...
  return (
    <>
      ...
      {/* Portal to document.body so the panel escapes the Header's stacking
          context — the RasterLegend below creates its own context via
          backdrop-filter, and z-index alone wasn't reliable. */}
      {open &&
        pos &&
        createPortal(
          <div
            ref={panelRef}
            className="p-3 rounded-lg min-w-[260px]"
            style={{
              position: 'fixed',
              top: pos.top,
              left: pos.left,
