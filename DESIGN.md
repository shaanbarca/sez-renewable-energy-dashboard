...
| Land-use override % (was "Usable ground %" / "WB buildout footprint") | 2 | 0.0–1.0, step 0.05 | 0.20 | **V3.9.1 haircut → V4.0.5 override (methodology #40).** Slider semantic switched from haircut (`effective = baseline × slider%`) to override (`effective = baseline + (hard_max - baseline) × slider%`). At slider=0% strict raster baseline; at slider=100% all soft-zoning exclusions overridden (only hard physical/legal constraints remain). Gates `within_boundary` grid_integration_category. Inline in Grid tab coverage panel + Resource tab Captive Solar → Ground-mounted subsection + Advanced Assumptions → Captive Solar accordion (`(global)` hint label flags the sync). See METHODOLOGY §5.1.1. |
| Rooftop layout density | 2 | 0.40–0.65, step 0.05 | 0.50 | **V4.1 RV15.** Fraction of usable roof covered by panels after spacing for shading, walkways, equipment. NREL TP-6A20-65298. Inline in Resource tab Captive Solar → Rooftop subsection + Advanced Assumptions → Captive Solar accordion. |
...
| **Captive Solar card (V4.0.5)** | Merged Rooftop + Ground-mounted card | Single `<StatCard>` with two muted-uppercase mini-headers (`ROOFTOP`, `GROUND-MOUNTED`) and a `TOTAL` row. Rooftop first. Each subsection shows "no data" placeholder when the underlying data is missing. Total row label composes the contents: `Captive MWp (rooftop + ground)` when both present, `(rooftop only)` / `(ground only)` when partial. Tooltip notes the rooftop/ground cost-tier mix (~5× $/MWp). Polygon source footer applies to both subsections. |
| `rooftop_layout_density` slider | Inline in Captive Solar → Rooftop subsection | V4.1 RV15 Tier 2 slider (0.40-0.65). Client-side rooftop MWp math mirrors `scorecard.py:348-353` for instant feedback; server still recomputes for RankedTable / RooftopPotentialTable. |
| `wb_buildout_footprint_ratio` slider | Inline in Captive Solar → Ground-mounted subsection | Same slider as Grid tab + Advanced Assumptions; `(global)` hint label flags the sync. Display values multiply client-side; backend uses the same value for the within_boundary gate. |
| `usable_roof_area_m2`, `building_count_standard_roof`, `building_data_confidence`, `building_data_reason_flagged` | Rooftop subsection metrics | Standard rooftops count + Usable roof area + Data confidence; reason text fills the placeholder when rooftop is missing |
| `within_boundary_capacity_mwp`, `within_boundary_area_ha`, `within_boundary_avg_pvout`, `within_boundary_generation_gwh`, `within_boundary_coverage_pct` | Ground-mounted subsection metrics (baseline values) | All baseline (raster) values; frontend computes deployable client-side as `baseline + (hard_max - baseline) × slider%` for instant slider feedback. PVOUT shared between baseline and override regions per methodology #40. |
| `within_boundary_hard_max_ha`, `within_boundary_capacity_hard_max_mwp` | Hard-max values (v4.0.5) | Polygon area / capacity passing only HARD filters (slope + Kawasan Hutan + peat). Slider override interpolates between baseline and these values. |
| `polygon_source_tier` | Polygon source footer | Bottom of merged Captive Solar card. Applies to both subsections (drives building detection + buildable area) |
| `buildable_area_ha`, `regional_groundmount_potential_mwp_50km` | Buildable Land card | Region-level (50 km) capacity; separate from the within-fence Captive Solar card |
...
| `wb_buildout_footprint_ratio` slider ("Land-use override % (global)") | Inline in coverage panel (+ Resource tab Captive Solar + Advanced Assumptions, V4.0.5) | Tier 2 slider in both Grid coverage-panel modes. Same Zustand state as the Resource tab Captive Solar ground-mounted slider and the Advanced Assumptions accordion slider — all three carry the `(global)` hint. V4.0.5 (methodology #40): semantic changed from haircut → override. Flips sites between self-sufficient and partial coverage by interpolating between raster baseline and hard_max. |
...
| 2026-05-11 | #36, #37 | **Captive Solar restructure (V4.0.5).** Merged ResourceTab's two StatCards (`Captive Solar (on-site)` ground-mount + `Rooftop Solar Potential`) into a single `Captive Solar` card with two muted-uppercase mini-headers (`ROOFTOP`, `GROUND-MOUNTED`) and a `TOTAL` row. Rooftop first per design review D2. Each subsection shows a "no data" placeholder if missing (D4); Total label composes the contents (`(rooftop + ground)` / `(rooftop only)` / `(ground only)`) per eng-review E2. Added client-side rooftop MWp math mirroring `scorecard.py:348-353` so the Rooftop Layout density slider feels instant alongside the ground-mounted slider (eng-review E1) — server still recomputes for RankedTable consistency. Renamed Advanced Assumptions accordion `Rooftop Solar (V4.1)` → `Captive Solar`, grouped 3 rooftop sliders under a Rooftop sub-header, added `wb_buildout_footprint_ratio` slider under a Ground-mounted sub-header (first time this slider is exposed in Advanced Assumptions). Relabelled `wb_buildout_footprint_ratio` from "WB buildout footprint (%)" → "Usable ground % (global)" in `src/dash/constants.py:202` so all 3 surfaces (Grid tab, Resource tab, Advanced Assumptions) carry the same label + `(global)` hint flagging the Zustand sync. | Builds on #36 (inline `rooftop_layout_density` slider already shipped). Issue body asked for a ground-mounted slider but the existing `wb_buildout_footprint_ratio` already covered that — the actual gap was discoverability + the merged Captive Solar mental model. /plan-design-review locked 7 design decisions (D1-D8), /plan-eng-review locked 3 eng decisions (E1-E3, including 1 critical-gap mitigation via `Number.isFinite` guard on client-side math). Master-plan ingestion deferred to future ticket. |
| 2026-05-11 | #40, #41 | **Buildability hard/soft filter split + slider override semantic (V4.0.5).** Replaces the V3.9.1 `wb_buildout_footprint_ratio` haircut with an interpolation override per methodology #40. Pipeline (`build_fct_site_resource.py`) gets a single-pass refactor producing both `filtered_mask` (full 4-layer pass — slope + Kawasan Hutan + peat + land cover) and `hard_filtered_mask` (HARD only — drops land cover). Two new CSV columns: `within_boundary_hard_max_ha`, `within_boundary_capacity_hard_max_mwp` (and pipeline-time scorecard adds `within_boundary_coverage_hard_max_pct`). Filter classification declared in `src/dash/constants.py::BUILDABILITY_LAYER_CLASSIFICATION`. Slider math (in `dash/logic/grid.py:112` + frontend `ResourceTab.tsx` + Ranked Table `columns.tsx`): `effective = baseline + (hard_max - baseline) × slider%`. At slider=0% strict raster baseline; at slider=100% all soft-zoning exclusions overridden. Slider relabelled "Usable ground %" → **"Land-use override % (global)"** with new description across `constants.py:202` + `AssumptionsPanel` + `ResourceTab`. Backend cost cascade reads slider-aware effective coverage via `ctx.grid_out["within_boundary_coverage_effective_pct"]`. METHODOLOGY §5.1.1 + DATA_DICTIONARY updated. Fleet-wide deployable capacity at default 20% slider: ~2,800 MWp (was ~300 MWp under V3.9.1 haircut) — closes the "industrial sites show ~0 captive" misleading display. New test file `tests/test_within_boundary_hard_soft.py` with 17 cases (pipeline invariants + slider math + 3-site regression locks). | Methodology proposal `docs/refinement/industrial_canopy_potential_methodology_2026-05-11.md` (rev 2). Source review anchored in NREL LA100 layered-potential methodology, LBNL urban-pavement studies, CA SB 49 50% canopy coverage policy. Eng review (D1A-D4A) locked: D1A all-consumer propagation, D2A silent semantic-change with changelog, D3A single-pass filter refactor, D4A full math + regression test coverage. Behavioral change: saved scenarios with `wb_buildout_footprint_ratio = 0.20` now produce ~10× larger captive numbers at fully-built industrial sites; KEK numbers shift modestly (raster baseline already captured most of polygon). |
