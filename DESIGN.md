...
| Usable ground % (was "WB buildout footprint") | 2 | 0.05–1.0, step 0.05 | 0.20 | **V3.9.1, relabelled V4.0.5.** Haircut applied to raw within-boundary buildable area. Gates `within_boundary` grid_integration_category. Inline in Grid tab coverage panel + Resource tab Captive Solar → Ground-mounted subsection + Advanced Assumptions → Captive Solar accordion (`(global)` hint label flags the sync). |
...
| `within_boundary_capacity_mwp`, `within_boundary_area_ha`, `within_boundary_avg_pvout`, `within_boundary_generation_gwh`, `within_boundary_coverage_pct` | Ground-mounted subsection metrics | All adjusted by the Usable ground % slider client-side; PVOUT is the only raw row (doesn't depend on the haircut). |
...
| `wb_buildout_footprint_ratio` slider | Inline in coverage panel (+ Resource tab Captive Solar + Advanced Assumptions, V4.0.5) | Tier 2 slider embedded in both Grid coverage-panel modes. Same Zustand state as the Resource tab Captive Solar ground-mounted slider and the Advanced Assumptions accordion slider — all three carry the `(global)` hint. Changes buildout haircut; flips sites between self-sufficient and partial coverage live. |
