...
def prepare_resource_df(tables: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    # v4.1b #93/#94: hydro proximity columns (optional — generated by
    # build_fct_hydro_proximity.py). Mirrors the geothermal pattern above.
    # Feeds the hydro-share leg of the 3-way hybrid optimizer (spec §6A.4)
    # and the dedicated Score Drawer hydro-proximity row.
    hydro_path = PROCESSED / "fct_hydro_proximity.csv"
    if hydro_path.exists():
        ...
        for col in [
            ...
            if col in hydro.columns and col not in resource.columns:
                hydro_cols.append(col)
        if len(hydro_cols) > 1:
            resource = resource.merge(hydro[hydro_cols], on="site_id", how="left")
