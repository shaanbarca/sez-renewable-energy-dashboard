...
This dashboard answers that question, site by site, across **81 sites**: 25 KEKs (Special Economic Zones) plus 56 industrial sites that sit outside KEK boundaries but drive the bulk of Indonesia's industrial CO₂ — Krakatau Steel, Inalum, Pupuk Kaltim, the 32 operating Java/Sumatra cement plants from the GEM tracker, and 10 non-KEK nickel IIA clusters (Weda Bay, Obi, Konawe and others, filtered from the CGSP tracker). Ammonia and petrochemical sites await a top-down universe-discovery pass before they're added — see TODOS M28/M29. It is the first tool that maps the intersection of:
...
*Last assessed: 2026-04-18 (M26 + M30: 81 sites via `build_industrial_sites.py` unioning GEM Cement/Steel + CGSP Nickel IIA + residual manual CSV — 7 rows of aluminium + fertilizer; sector-intensity demand; dual-mode captive/CBAM via SiteTypeConfig registry; SectorSummaryChart; M30 CBAM Scope 2 savings bound to electric-share RE-addressable fraction). Ammonia + petrochemical scaffolding ready in registry pending TODOS M28/M29 universe-discovery pass. Re-assess after each major pipeline change.*
...
| ✅ DONE | **WACC expansion 4–20%** — `WACC_VALUES` now `[4, 6, 8, 10, 12, 14, 16, 18, 20]`; 1,458 rows in `fct_lcoe` (81 sites × 9 WACC × 2 scenarios) | P1 | Done |
| ✅ DONE | **Industrial parks expansion** — 56 industrial sites (32 cement, 7 steel, 10 nickel IIA, 2 aluminium, 5 fertilizer) added alongside 25 KEKs, 81 total. Site selection itself is pipeline-driven: GEM/CGSP trackers inside `build_industrial_sites.py`, with a small residual manual CSV (7 rows) for sectors without a tracker. Fertilizer universe closed 2026-04-18 via 4-source discovery gate (M26 complete). Sector-intensity demand method replaces area-proxy for standalone/cluster sites. CBAM exposure 68/81 sites (12 KEK via 3-signal + 56 industrial via direct: 32 cement + 17 iron_steel + 5 fertilizer + 2 aluminium). Ammonia + petrochemical sector scaffolding (enum, intensity, CBAM constants, frontend colors) is in place pending top-down universe discovery (TODOS M28/M29). Dual-mode captive matching (proximity vs direct). `SiteTypeConfig` registry makes adding new types a 1-file change. | P1, P2, P3, P4, P5 | Done (V4.1) |
| ✅ DONE | **CBAM Scope 2 RE savings bounded by RE-addressable fraction (M30)** — `CBAM_RE_ADDRESSABLE_FRACTION` added to `src/assumptions.py` (cement 0.12, fertilizer/ammonia 0.10, steel_bfbof 0.80, else 1.0). Wired into `src/dash/logic/cbam.py::compute_cbam_trajectory`. Scope 1 cost unchanged; cement 2034 RE savings drop from ~63 USD/t to ~7.6 USD/t. | P1, P2, P3 | Done (2026-04-18) |
