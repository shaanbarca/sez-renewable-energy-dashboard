...
| M26 | **Fertilizer universe expansion (5 Pupuk subsidiaries, not 3)** | Industrial Parks Expansion (2026-04-17); revised 2026-04-18 | P1, P2 | Current residual CSV has 3 fertilizer rows (Pupuk Kaltim Bontang, Pusri Palembang, Petrokimia Gresik) but **misses Pupuk Kujang (Cikampek, W. Java) and Pupuk Iskandar Muda / PIM (Lhokseumawe, Aceh)** — both wholly-owned Pupuk Indonesia subsidiaries producing ammonia + urea. Root cause: hand-picking without a universe-discovery step. Resolution plan: execute the 4-source intersection gate (state holdings + APPI + ESDM gas allocations + BPS/Comtrade) in [docs/cbam_sector_data_collection_plan.md](docs/cbam_sector_data_collection_plan.md) §3.0 before geocoding. Expected output: 5 confirmed rows (79 sites → 84). Supersedes the earlier "tracker or residual CSV" framing. |
...
| M30 | **CBAM `re_addressable_fraction` — bound Scope 2 RE savings to electricity-only share** | 2026-04-18 | P1, P2, P3 | Current `CBAM_ELECTRICITY_INTENSITY_MWH_PER_TONNE` is thermal-inclusive for cement (0.9 vs 0.11 electricity-only) and ammonia/fertilizer (10 vs 1.0 electricity-only) — see explicit comment at `src/assumptions.py:622-625`. `savings = scope2 × rate` therefore treats kiln gas / SMR feedstock as if it were grid electricity that vanishes with solar. Fix: add `CBAM_RE_ADDRESSABLE_FRACTION` dict (cement ~0.12, fertilizer ~0.10, ammonia ~0.10, others ~1.0) and multiply Scope 2 savings by the sector's fraction in `src/dash/logic/cbam.py`. Effect: cement RE savings at the 32 existing cement sites drop ~88% (2030 savings ~$32/t → ~$4/t). Scope 1 separation already correct, unaffected. Ship alongside M26+M28 fertilizer site additions in one coherent change — CBAM outputs not yet externally circulated, no user re-education needed. Full methodology: [docs/cbam_sector_data_collection_plan.md](docs/cbam_sector_data_collection_plan.md) §4.1. |
...
| ✅ | Ammonia + petrochemical scaffolding | 2026-04-18 | `Sector` enum extended with `ammonia` and `petrochemical`. Demand intensities + reliability defaults wired in. CBAM calibrated to Indonesia-specific ammonia Scope 1 = 2.3 tCO₂/t (ICGD gas-SMR). Frontend rollup colors reserved (ammonia teal #26A69A, petrochemical purple #7E57C2). Site rows deferred to M28/M29 — ad-hoc hand-picking would miss obvious producers (Pupuk Kaltim Bontang, Indonesia's largest ammonia plant at ~3.4 Mt/yr) so we want a top-down universe-discovery pass first. Site count unchanged (still 79). |
