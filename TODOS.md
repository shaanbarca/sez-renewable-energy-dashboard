...
| M31 | **Repoint action flags / economic tier / competitive gap from `lcoe_mid` to `delivered_cost_blended`** | V3.10 deferred (2026-04-21) | P1, P2, P4 | V3.10 added `delivered_cost_blended_usd_mwh` as an additive tenant-view metric (see METHODOLOGY §5.4). Open question: should downstream mechanics (`action_flags`, `economic_tier`, `solar_competitive_gap_pct`, `carbon_breakeven_usd_tco2`) switch from the gate-driven `lcoe_mid_usd_mwh` (T1) to the blended number (T3)? Partial-captive KEKs (Galang Batang, Batang, Gresik, Kendal, Lido) currently report +35-59% above-grid on `lcoe_mid` but only 0.2-4.5% on delivered — the action-flag classification changes materially. Revisit once PR2 lands and numbers are visible side-by-side in the UI. **Prerequisite:** rename + enum decisions in [docs/TAXONOMY.md](docs/TAXONOMY.md) §7.1–§7.2 should land first so the flag logic doesn't bake in names that change. Full deferred-decision context in TAXONOMY.md §7.3. |
