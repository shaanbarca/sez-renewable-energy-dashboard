...
| M31 | **`CostBasis` toggle: user picks which cost layer feeds action flags / economic tier / competitive gap** | V3.10 deferred, reframed 2026-04-21 | P1, P2, P4 | V3.10 added `delivered_cost_blended_usd_mwh` as an additive tenant-view metric (METHODOLOGY §5.4). **Reframed from one-way repoint to user toggle:** instead of swapping `lcoe_mid` (T1) for `delivered_cost` (T3) globally, add a `CostBasis` toggle (raw / firmed / delivered) parallel to the existing `BenchmarkMode` and `EnergyMode` toggles. Different personas want different lenses — energy economist → raw, DFI → firmed, industrial tenant → delivered. Partial-captive KEKs (Galang Batang, Batang, Gresik, Kendal, Lido) currently report +35-59% above-grid on `lcoe_mid` but only 0.2-4.5% on delivered — the action flag changes materially, but whether that's "correct" depends on whose question you're answering. Let the user pick. **Scope:** `CostBasis` StrEnum + `(EnergyMode × CostBasis)` resolver in `basic_model.py`, nested `action_flags_by_basis` / `economic_tier_by_basis` dicts precomputed in scorecard, UI toggle in AssumptionsPanel, map legend shows active basis, methodology §5+§7 rewrite, golden fixture. 3–4 engineering days. **CBAM is a `BenchmarkMode` extension (`bpp_cbam_adjusted`), not a fourth basis — cost side vs benchmark side stay orthogonal.** Full spec: [docs/TAXONOMY.md](docs/TAXONOMY.md) §7.3 (matrix, defaults, rejected alternatives). **Prerequisite:** §7.1 delivered_cost rename should land first; §6.5 `CostBasis` enum ships with this PR. |
